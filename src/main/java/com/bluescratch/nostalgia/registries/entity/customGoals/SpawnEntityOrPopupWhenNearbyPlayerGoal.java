                ...
                for (var level : levels) {
                    for (var serverPlayer : level.players()) {
                        NostalgiaNetwork.sendToPlayer(
                                serverPlayer,
                                new PopupShowPacket(true, PopupBaseValues.popupVisibleName, RandomPopupDialogue.values()[mob.level().random.nextInt(RandomPopupDialogue.values().length)].getText())
                        );
